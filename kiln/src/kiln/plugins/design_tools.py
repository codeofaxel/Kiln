...
class _DesignToolsPlugin:
    ...
    def register(self, mcp: Any) -> None:
        ...
        @mcp.tool()
        def set_design_color(recipe_path: str, color: str) -> dict:
            """Record a single-color design's intended color on its recipe (free).

            Sets the color of a single-part design and saves it as a new recipe
            version — no re-slice, because a solid color simply IS the loaded
            filament.  This is the lightweight, free way for a color picker or a
            "make it teal" request to persist the choice on the design's recipe,
            the real versioned source of truth (the same rails every other edit
            rides).

            For a MULTI-part design (different colors on different regions), use
            ``change_part_color`` — it re-slices per part and is a Pro feature.

            :param recipe_path: Path to the recipe directory (the folder that
                holds ``.kiln_recipe.json``).
            :param color: A color name (``"teal"``) or hex (``"#16B2A3"``; an
                8-digit AMS ``"#RRGGBBAA"`` is accepted, alpha dropped).
            :returns: Dict with the recorded color, the part name, and the new
                recipe version.
            """
            ...
            try:
                ...
                if recipe_file is None:
                    return {
                        "success": False,
                        "error": f"No design recipe found at {recipe_path!r}.",
                    }
                recipe = load_recipe(recipe_file)
                if not recipe.parts:
                    return {"success": False, "error": "Recipe has no parts to color."}
                if len(recipe.parts) > 1:
                    return {
                        "success": False,
                        "error": (
                            "This design has multiple parts — use change_part_color "
                            "to color a specific part (a Pro feature)."
                        ),
                        "part_names": [p.name for p in recipe.parts],
                    }

                part = recipe.parts[0]
                old_color = part.color
                # Tidy a hex value; pass a color name straight through.
                new_color = normalize_hex_color(color) or color.strip()
                if not new_color:
                    return {"success": False, "error": "color must not be empty."}

                # Reuse the canonical version-bump + color-mutation helpers; this
                # tool adds no new versioning or color logic of its own.
                bumped = create_new_version(
                    recipe,
                    recipe_file,
                    changes={f"{part.name}.color": f"{old_color} -> {new_color}"},
                )
                final = update_part_color(bumped, part.name, new_color)
                save_recipe(final, recipe_path)
                ...
                return {
                    ...
                    "message": f"Set {recipe.name} to {new_color} (v{final.version}).",
                }
            except Exception as exc:
                _logger.error("set_design_color failed: %s", exc, exc_info=True)
                return {"success": False, "error": str(exc)}
