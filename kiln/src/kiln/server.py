...
def dialog_offers() -> tuple[bool, bool]:
    ...
    One printer is every tier's.  "Every printer" is offered on top only
    when the tier runs several printers at once (a per-request read on
    the hosted server) — the one grant-time tier line, the same one the
    writer enforces.  Where a print may START from (at home, or away
    through Kiln's cloud) is judged at each start, not here.  Offering
    what will not open is a form that lies.
    """
...
def _no_yes_message(tool_name: str, file_name: str, aimed: str) -> str:
    ...
    so the sentence there names the account's doors instead: the person's
    Approve on the print page, or a delegation the person granted this
    agent — in this app's dialog when it draws one, or on their account
    page (public Kiln ships the hook; kiln-pro keeps the record).
...
def start_print(
    ...
    try:
        ...
        nozzle_unchecked_line: dict[str, Any] | None = None
        try:
            from kiln import _pro_nozzle_bridge

            # Asked whether or not kiln-pro is installed here: the bridge
            # answers locally when it is and from Kiln's servers when it is
            # not, and says why when neither could.  A gate on a local
            # install here is how this check ran for almost nobody.
        ...
        if nozzle_unchecked_line is not None:
            out["nozzle_check"] = nozzle_unchecked_line
...
def preflight_check(
    ...
    try:
        ...
        # kiln-pro are asked through Kiln's servers, and told why when
        # nothing answered (a check the list names as not made).  When the verdict surfaces, it joins the checks list
        ...
        try:
            ...
            if not _planned_grams and file_path:
                # The file itself says what it will use: the slicer's own
                # grams line, read the way the cutter count reads it.  The
                # validation result never carried a figure, so this check
                # could not fire for anyone until it read the file.
                from kiln._pro_cutter_bridge import grams_in_file

                _planned_grams = float(grams_in_file(str(file_path)) or 0)
            ...
            if _printer_id and _planned_grams > 0:
                _nozzle_verdict = _pro_nozzle_bridge.consult_capacity(
                    ...
                    printer_model=_pf_model or "",
                )
                if _nozzle_verdict is not None and _nozzle_verdict.get("status") not in (None, "unknown_baseline", "unknown_nozzle", "unknown_usage", "invalid_input"):
                    _nz_check: dict[str, Any] = {
                    ...
                    if _nozzle_verdict.get("upgrade_hint"):
                        _nz_check["upgrade_hint"] = _nozzle_verdict["upgrade_hint"]
                    checks.append(_nz_check)
                elif _nozzle_verdict is None:
                    # Kiln could not ASK: the list says so, the same way it
                    # does for the blade, rather than reading as "fine".
                    _nz_gap = _pro_nozzle_bridge.nozzle_unchecked(_printer_id)
...
def send_gcode(commands: str, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        # -- Mid-print Z-homing gate -------------------------------------
        # A bare G28 (or any G28 naming Z) descends Z toward the bed at
        # the machine's homing XY.  While a job is printing or paused the
        # bed carries a partial print, so that descent is a nozzle
        # collision waiting for the homing point to coincide with the
        # part.  The safe shape is a relative lift then "G28 X Y"
        # (kiln.printers.safe_motion).  State is queried live; if the
        # printer cannot be asked, the gate fails open — it exists to
        # stop a proven hazard, not to add a new way to be offline.
        _skip_midprint_home = os.environ.get(
            "KILN_SKIP_MIDPRINT_HOMING_CHECK", ""
        ).strip() in ("1", "true", "yes")
        if not _skip_midprint_home:
            from kiln.printers.safe_motion import homes_z as _homes_z

            _z_home_cmds = [c for c in cmd_list if _homes_z(c)]
            if _z_home_cmds:
                _live_state = None
                try:
                ...
                if _live_state in (PrinterStatus.PRINTING, PrinterStatus.PAUSED):
                    _audit(
                        "send_gcode",
                        "midprint_z_home_blocked",
                        details={
                            "state": _live_state.value,
                            "commands": _z_home_cmds[:5],
                        },
                    )
                    _record_tool_block("send_gcode")
                    return {
                        "success": False,
                        "error": {
                            "code": "GCODE_MIDPRINT_Z_HOME",
                            "message": (
                                f"Refused to home Z while the printer is "
                                f"{_live_state.value}: the bed carries a "
                                f"partial print and Z homing descends the "
                                f"nozzle toward it.  Offending: "
                                f"{'; '.join(_z_home_cmds[:3])}.  Lift first "
                                f"and home X/Y only (G91, G1 Z5 F600, G90, "
                                f"G28 X Y).  Bypass with "
                                f"KILN_SKIP_MIDPRINT_HOMING_CHECK=1 if the "
                                f"bed is genuinely clear."
                            ),
                        },
                        "blocked_commands": _z_home_cmds,
                    }
...
# What an OFFLINE person is told per pro tool: the tool's kind (verdict /
# made / record / read / sync / action), from the manifest's ``offline``
# block, filled in by the same loop.  The kind decides what is on the line
# when Kiln's servers cannot be reached -- a verdict not given is never a
# yes, a record not written must be told again -- and the private side
# resolves it for every tool at ship time (its manifest gate refuses a tool
# it cannot place).  A lookup miss means the bundle predates the block, and
# ``kiln.served_answer`` derives the kind by name the same way.
_PRO_TOOL_OFFLINE_KIND: dict[str, str] = {}
_OFFLINE_SCHEMA_VERSION = 1
...
def _pro_api_call(tool_name: str, _timeout: float = 30.0, **kwargs) -> dict:
    ...
    When no answer comes back, the envelope says which of four things
    happened, in ``why`` (``offline`` / ``signed_out`` / ``unanswered`` /
    ``refused``, see :mod:`kiln.served_answer`) beside a sentence a person
    can act on; a server body that already carries a code and a sentence
    of its own comes back untouched.
    """
...
def _register_pro_tool_stubs(mcp_instance) -> None:
    ...
    for tool_def in tools:
        ...
        # What an offline person is told: the tool's kind, one word, read
        # only from a block of the version this build understands and only
        # when it names a kind this build can word.  Anything else leaves
        # the entry absent and the kind derived by name at call time.
        offline_def = tool_def.get("offline")
        if (
            isinstance(offline_def, dict)
            and offline_def.get("schema_version") == _OFFLINE_SCHEMA_VERSION
        ):
            from kiln.served_answer import KINDS as _OFFLINE_KINDS

            offline_kind = str(offline_def.get("kind") or "").strip()
