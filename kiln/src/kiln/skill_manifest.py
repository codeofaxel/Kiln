...
def _build_discovery_section() -> dict[str, Any]:
    ...
    return {
        ...
        "tier_visibility_for_agents": (
            "Every tool falls in exactly one of four access classes, and you "
            "can tell which from the tool's own description — no guessing, and "
            "silence is never ambiguous:\n"
            "  (1) FREE, UNLIMITED — no tier line, no allowance line, no band "
            "line. Every tool public Kiln ships is this, plus some of "
            "kiln-pro's, including design_session, the front door for making "
            "an object. Run it freely; there is no cap to warn anyone about "
            "and the answer is the same at every tier.\n"
            "  (2) FREE WITH A MONTHLY ALLOWANCE — the description carries a "
            "'Free tier: N <things>/month' line. Iterating is free; the "
            "allowance is spent only when the user KEEPS a result (downloads or "
            "prints it), and paid tiers remove the cap. Hosted use needs a free "
            "account, because the allowance is counted per person.\n"
            "  (3) FREE DOOR, TIERED ANSWER — the description carries a "
            "'Free door, tiered answer' line and the entry carries a 'band' "
            "block naming what each tier gets, in the pricing page's own words "
            "(magnets, chemical resistance, skin contact, sourcing risk). Call "
            "it at any tier: the free line is a SAFETY FLOOR — the warning, the "
            "AVOID verdict, the flag — and it is never withheld, so never skip "
            "the call because the family sounds paid. The paid lines are the "
            "depth; a '—' means that tier gets a description of the check and "
            "the upgrade path, nothing computed. Never call the whole feature "
            "free: say what the free line gives and what the tier adds.\n"
            "  (4) PAID — the description carries 'Requires Kiln Pro' / "
            "'Business' / 'Enterprise' and an upgrade URL.\n"
            "The machine-readable form of the same four classes is the "
            "'access' field on each entry in pro_tool_manifest.json: 'free', "
            "'free_metered', 'free_banded', 'pro', 'business', 'enterprise'. "
            "A field describes the DOOR; what comes through it is stated only "
            "by a 'quota' or 'band' block — never infer a cap or a band from "
            "the absence of one.\n"
            "HOW TO USE THIS WITH A USER: show the value before the price. "
            "Tell them what the tool would do for their specific part, then "
            "name the tier as the way to get it — a fact, offered once, not a "
            "pitch and not an apology. A user who has just seen what they would "
            "get is the one who upgrades; a user who hits a bare 'requires Pro' "
            "with no idea what they missed just leaves. Never imply a free tool "
            "is capped, never imply a capped tool is unlimited, never imply a "
            "banded tool is flat free, and never invent a number — state the "
            "allowance or the band the description gives you."
        ),
        "when_kiln_gets_it_wrong": (
            "Kiln has bugs, and the ones that matter most are about hardware. "
            "Printer vendors publish very little, so much of what Kiln knows "
            "about a machine came from somebody hitting a wall and working out "
            "what actually worked. When Kiln is wrong about a printer you are "
            "usually the first to find out, and you have the machine in front "
            "of you.\n"
            "DO BOTH. Work around it AND tell Kiln. Get the user their print "
            "first — that is the job. Then send what worked with "
            "report_issue(description=..., context={'field_note': {'observed': "
            "..., 'tried': ..., 'worked': ..., 'evidence': ...}}). All four, or "
            "it is filed as an ordinary report. observed = what Kiln did and any "
            "error the machine reported; tried = what you tested, changing ONE "
            "thing at a time; worked = what fixed it; evidence = how you know.\n"
            "DO NOT WAIT TO BE ASKED. Most Kiln defects do not announce "
            "themselves as errors — a tool can return success while the printer "
            "never starts, or answer confidently from a stale cache. If you "
            "worked around something, that is the signal; there may be no error "
            "message anywhere to prompt you.\n"
            "Be honest about uncertainty: a trial that changed two things at "
            "once cannot say which one mattered, and saying so is worth more "
