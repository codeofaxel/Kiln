...
class OpenSCADProvider(GenerationProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        Curves are cut by Kiln's rule (:func:`kiln.curve_resolution.apply_rule`)
        unless the code sets its own resolution at the top level.

        Args:
            prompt: Valid OpenSCAD code.
            format: Output format (only ``"stl"`` supported).
            style: Ignored for OpenSCAD.
            as_written: Compile the code exactly as given, rule and all
                left out -- for source a design already keeps, so a
                rebuild makes the part it made the first time.
    ...
    def validate_scad(self, code: str) -> dict[str, Any]:
        ...
        """
        # Validate what compile would build: the same rule, the same file.
        from kiln.curve_resolution import apply_rule
...
def compose_from_primitives(
    ...
    *,
    ...
    # Curves follow Kiln's rule (kiln.curve_resolution), written into the
    # file; only the hex prism keeps a typed count -- it IS a hexagon.
    from kiln.curve_resolution import apply_rule

    scad_code = apply_rule("\n".join(scad_lines))
