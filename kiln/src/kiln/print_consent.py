...
The dialog can carry that purpose.  Its main field is a choice — this
print only, this print and the next two hours, this print and the rest
of today, or no — beside a length the person may type and, on the fleet
tier, this printer or every printer.  The choice is the person's,
...
doors a window can be opened through (the first is a terminal; on the
hosted server the account's own store, which kiln-pro's delegation
record backs).  A person who never opens a terminal gets the window
feature this way; nothing here gives an agent a way to open one.
