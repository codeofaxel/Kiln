...
rule (it scans the whole tracked tree and commit messages); see
...
standards / plain contract notes / fake secrets / mock hosts).
...
def _live_run() -> subprocess.CompletedProcess:
    """One full-tree run of the gate, shared by the live-tree tests."""
    root = Path(__file__).resolve().parents[2]
...
def test_scope_covers_public_tests() -> None:
    ...
    assert _rules("kiln/tests/test_moat_comment_leak.py", "# moat moat kiln_pro/data/x.json\n") == []
...
def test_freezes_dotted_kiln_pro_paths() -> None:
    ...
    new = [leak for leak in leaks if leak[2] == "new kiln_pro path"][0]
    ...
    assert [leak for leak in leaks if leak[2] == "new kiln_pro path"] == []
