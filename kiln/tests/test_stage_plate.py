...
class _SliceResult:
    """A finished slice_model call, as the lowlevel hook sees it."""
...
class TestASliceShowsThePlateAsItWillPrint:
    """The slice doors are on the roster, so what does their panel show?
    Not the input (disqualified), not the wrapped ``.gcode.3mf`` (an
    archive, ranked as the product) — the mesh the slicer was handed,
    standing on this install's bed, dressed in the skirt and the prime
    tower the slice added.  The jar's 30 mm tower was never in any
    preview until this."""

    def test_the_stage_names_the_sliced_mesh_and_carries_the_slicers_additions(
        ...
        monkeypatch.setattr(
            "kiln.printer_model_resolver.resolve_printer_model", lambda: "bambu_a1"
        )
        local_stage._reset_for_tests()

        cube = _real_cube(tmp_path / "cube.stl")          # 20 mm, centred on the origin
        gcode = tmp_path / "cube.gcode"
        lines = ["; generated by OrcaSlicer 2.3.2 on 2026-09-21", "G90", "M83", "G28"]
        for i in range(2):
            z = 0.2 * (i + 1)
            lines += [";LAYER_CHANGE", f";Z:{z}"]
            if i == 0:
                lines += [";TYPE:Skirt", _loop(116, 116, 140, 140, z)]
            lines += [";TYPE:Outer wall", _loop(118, 118, 138, 138, z)]
            lines += [";TYPE:Prime tower", _loop(150, 118, 160, 128, z)]
        gcode.write_text("\n".join(lines) + "\n")
        monitor_twin.note_sliced(cube, str(gcode))
        wrap = tmp_path / "cube.gcode.3mf"
        wrap.write_bytes(b"PK\x03\x04not-a-real-archive")

        token = local_stage.token_for_call_result(_SliceResult({
            "success": True,
            "input_path": cube,
            "output_path": str(wrap),
            "output_3mf_path": str(wrap),
            "raw_gcode_path": str(gcode),
            "stage_mesh_path": cube,
        }))
        assert token and local_stage.resolve(token) == cube

        payload = local_stage._payload_for_mesh(cube)
        assert payload["plate"]["printer_id"] == "bambu_a1"
        classes = {f["class"] for f in payload["slicer"]["features"]}
        assert {"skirt", "prime_tower"} <= classes, classes
