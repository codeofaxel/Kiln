...
class TestTheResolver:
    ...
    def test_a_profile_that_states_its_density_is_believed_outright(self):
        from kiln.slicer_filament import resolve_slice_filament

        f = resolve_slice_filament(
            "PETG", settings={"filament_density": "1.32", "filament_type": "PLA-SILK"},
        )
        assert (f.density_g_per_cm3, f.source) == (1.32, "profile")
...
class TestTheLoadedSpool:
    def _adapter(self, tray_now: str, trays: list[tuple[int, str]]):
        return SimpleNamespace(
            get_ams_status=lambda: {
                "tray_now": tray_now,
                "units": [{"unit_id": 0, "trays": [
                    {"slot": s, "tray_type": t, "tray_color": "FFFFFFFF", "remain": 50}
                    for s, t in trays
                ]}],
            },
        )
    ...
    def test_the_first_loaded_tray_when_the_unit_names_no_active_one(self):
        """The A1 keeps tray_now at 255 with trays loaded."""
        from kiln.slicer_filament import loaded_filament_type

        assert loaded_filament_type(self._adapter("255", [(1, "ABS"), (3, "PLA")])) == "ABS"
...
class TestSliceFileCarriesTheIdentity:
    ...
    def test_a_profile_that_already_states_a_density_goes_to_the_slicer_untouched(self, tmp_path):
        """The author spoke: a user's own filament profile is not rewritten,
        and the response says the density came from the profile."""
        stl = _cube(tmp_path / "cube.stl")
    ...
    def test_the_slice_is_still_counted_against_the_printer_profile(self, tmp_path):
        ...
        with patch("kiln.slicer._record_slice", side_effect=lambda p, *a, **k: recorded.append(p)):
            _run_prusa(stl, {}, profile=resolve_slicer_profile("bambu_a1"), material="PETG")
        assert recorded and Path(recorded[0]).name.startswith("bambu_a1_")

    def test_estimate_print_hands_its_material_to_the_slice(self, tmp_path):
        from kiln.slicer import estimate_print

        stl = _cube(tmp_path / "cube.stl")
        with patch("kiln.slicer.slice_file") as spy:
            spy.return_value = SliceResult(success=True, output_path=str(tmp_path / "none.gcode"))
            estimate_print(stl, material="ASA")
...
class TestTheDoors:
    ...
    def test_the_pipelines_read_the_loaded_spool_from_the_adapter(self, tmp_path):
        ...
        with patch("kiln.slicer.slice_file", spy), \
            ...
            quick_print(model_path=stl, printer_id="ender3", skip_validation=True)
        assert spy.call_args.kwargs["material"] is None
...
class TestTheCliDoor:
    ...
    def _invoke(self, tmp_path, args: list[str], tracker_material: str | None = None):
        ...
        spy, _ = _fake_slice(tmp_path)
        tracker = MagicMock()
        tracker.get_material.return_value = (
            SimpleNamespace(material_type=tracker_material) if tracker_material else None
        )
        with patch("kiln.cli.main._autodetect_printer_profile_id", return_value=None), \
                ...
                patch.dict(os.environ, {"KILN_MATERIAL": "", "KILN_DEFAULT_MATERIAL": "", "KILN_FILAMENT": ""}), \
    ...
    def test_the_tracked_spool_is_the_loaded_one(self, tmp_path):
...
class TestPrusaSlicerWritesTheWeightItself:
    ...
    def test_a_multi_extruder_profile_takes_the_single_density(self, tmp_path):
        """PrusaSlicer reads a per-extruder value off the first entry when a
        vector is short, so the scalar the invariant writes serves every slot."""
        from kiln.slicer_profiles import resolve_multiextruder_profile

        stl = _cube(tmp_path / "cube.stl")
        result = slice_file(
            stl, profile=resolve_multiextruder_profile("bambu_a1", 4), slicer_path=_real_prusaslicer(),
