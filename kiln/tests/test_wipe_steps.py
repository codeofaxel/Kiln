...
class TestStepMode:
    ...
    def test_the_last_step_runs_the_finish_once_and_reports_the_rest(self, bambu, monkeypatch):
        ...
        # The finish runs from a thread the request cannot take with it, so
        # the fan-off may already have landed by the time the answer is read:
        # settle first, then judge the whole tail in order.
        from kiln.printers import routine_ledger

        assert routine_ledger.wait_settled(5.0)
        scripts = _scripts(bambu)
        # heater off, the plan's cool-down begins, and the fan goes off once it ends
        assert scripts[-3:] == ["M104 S0", "M106 S255", "M106 S0"]
