...
class TestPerModelTemplateSelection:
    ...
    def test_an_uncaptured_nozzle_keeps_its_own_machines_warm_up_and_says_so(self):
        """A 0.6 owner must never be handed a 0.4 sequence silently -- and
        must never be handed ANOTHER MACHINE'S.  This used to fall back to
        the A1's, which is also a 0.4 sequence, and one that disables the
        soft endstops and drives X to -48.2 on a machine whose own warm-up
        never goes below X100."""
        from kiln.printers.bambu_3mf import (
            _MODEL_START_GCODE_FILES,
            _load_a1_start_gcode,
        ...
        gcode, source, nozzle = _start_gcode_choice("bambu_p2s", 0.6)
        assert (source, nozzle) == ("bambu_p2s", "0.4"), "its own machine, at the size that was captured"
        assert gcode == _load_model_template(_MODEL_START_GCODE_FILES[("bambu_p2s", "0.4")])
    ...
    def test_no_enclosed_models_warm_up_drives_x_negative_at_any_nozzle(self, model, nozzle):
        ...
        substitution was removed for the 0.4 captures and left in place for
        every other nozzle -- the same machine, the same frame, one size
        over."""
        from kiln.printers.bambu_3mf import _select_start_gcode

        gcode, source = _select_start_gcode(model, nozzle)
        assert source == model, f"{model} at {nozzle} mm was served {source}'s warm-up"
        assert not re.search(r"X-\d", gcode), f"{model} at {nozzle} mm drives X negative"

    def test_a_bed_slinger_at_another_nozzle_keeps_its_own_chute_not_the_a1s(self):
        """The A1 mini's chute is at X-30 on a 180 mm plate.  At 0.6 mm it
        used to get the A1's -- X-48.2, and a warm-up cut for a 256 mm plate
        that travels to Y262.5 on a machine 180 mm deep."""
        from kiln.printers.bambu_3mf import _select_start_gcode

        gcode, source = _select_start_gcode("bambu_a1_mini", 0.6)
        assert source == "bambu_a1_mini"
        xs = [float(v) for v in re.findall(r"^\s*G[01]\b[^;\n]*\bX(-?\d+\.?\d*)", gcode, re.M)]
        ys = [float(v) for v in re.findall(r"^\s*G[01]\b[^;\n]*\bY(-?\d+\.?\d*)", gcode, re.M)]
        assert min(xs) > -48.2 and max(ys) < 200.0

    def test_the_result_says_when_the_warm_up_is_another_nozzles(self, tmp_path):
        from kiln.printers.bambu_3mf import BambuPrintSettings, build_bambu_3mf

        gcode = "; generated by PrusaSlicer\n;LAYER_CHANGE\n;Z:0.2\nG1 X10 Y10 Z0.2 E0.1\n"
        borrowed = build_bambu_3mf(
            gcode, str(tmp_path / "p2s_06.gcode.3mf"),
            settings=BambuPrintSettings(nozzle_diameter=0.6), printer_model="bambu_p2s",
        )
        assert (borrowed.start_gcode_model, borrowed.start_gcode_nozzle) == ("bambu_p2s", "0.4")
        said = borrowed.start_gcode_warning
        assert said and "0.4 mm" in said and "0.6 mm" in said and "own" in said
        assert borrowed.to_dict()["start_gcode_nozzle"] == "0.4" and "start_gcode_warning" in borrowed.to_dict()

        exact = build_bambu_3mf(
            gcode, str(tmp_path / "p2s_04.gcode.3mf"),
            settings=BambuPrintSettings(nozzle_diameter=0.4), printer_model="bambu_p2s",
        )
...
class TestEndTemplateExpansion:
    ...
    @staticmethod
    def _end_lifts_of_a_built_file(tmp_path, height: float, printer_model: str | None) -> list[float]:
        """Every absolute Z in the end block of a file built the way a print
        is built, for a part *height* tall."""
        body = (
            "; generated by PrusaSlicer\nM83\n"
            ";BEFORE_LAYER_CHANGE\n;Z:0.2\n;LAYER_CHANGE\nG1 Z0.2 F600\nG1 X10 Y10 E0.5\n"
            f";BEFORE_LAYER_CHANGE\n;Z:{height:g}\n;LAYER_CHANGE\nG1 Z{height:g} F600\nG1 X20 Y20 E0.5\n"
        )
        out = tmp_path / f"tall_{height:g}.gcode.3mf"
        build_bambu_3mf(body, str(out), printer_model=printer_model)
        with zipfile.ZipFile(out) as zf:
            gcode = zf.read("Metadata/plate_1.gcode").decode("utf-8")
        end = gcode[gcode.rindex(";===== date:"):]
        zs, absolute = [], True
    ...
    @pytest.mark.parametrize("height", [251.0, 253.0, 255.0])
    def test_the_first_lift_never_goes_past_the_machines_own_top(self, tmp_path, height):
        """Kiln lifts max_z + 5 after the last layer; within 5 mm of the A1's
        256 mm that asked for up to Z260, after a warm-up that switched the
        soft endstops off.  Built through the same door every print uses."""
        zs = self._end_lifts_of_a_built_file(tmp_path, height, "bambu_a1")
        assert zs and max(zs) <= 256.0, f"a {height:g} mm A1 print sends the head to Z{max(zs):g}"
        assert all(z >= height for z in zs), "and never below the part"

    def test_a_machine_kiln_does_not_know_gets_no_invented_ceiling(self, tmp_path):
        """An unknown model borrows the A1's end sequence but not the A1's
        height: its first lift is max_z + 5, uncapped, because its top is not
        on record.  (The borrowed sequence still parks where Bambu's A1
        template does, at 256 -- a machine taller than an A1 needs its own
        end sequence, which is why every model in the catalogue has one.)"""
        zs = self._end_lifts_of_a_built_file(tmp_path, 300.0, "bambu_never_heard_of_it")
        assert zs[0] == 305.0, "max_z + 5, uncapped, where the machine's top is not on record"
