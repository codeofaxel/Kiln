...
# M115 @ 2.1.2.4: the identity line, then one Cap: line per capability; Z_PROBE means a bed probe is built in.
M115_2X = (
...
# 1.1.9.1: the LCD-language report string; "(Github)" as the build source, no build stamp, no Cap: lines.
...
# Creality Ender-3 1.1.6.1 tree: the detailed build version is "Creality 3D" -- no version number at all.
...
# Creality Ender-3 V2 tree (2.0.8.x): the short build version is "V1.0.4", with a 2.x build stamp.
...
# Prusa-Firmware 3.14.1: a FIRMWARE_URL field, no UUID, no Z_PROBE capability.
...
# M211, 2.0.9.2 and later: a replayable line, then both triples
...
# M211, 2.0.0 to 2.0.9.1 and the Ender-3 V2 / AnkerMake M5 trees: one echo line.
M211_20X = "echo:Soft endstops: ON  Min:  X0.00 Y0.00 Z0.00   Max:  X220.00 Y220.00 Z250.00\nok"
# M211, 1.1.9.1: the state word is "On " (trailing space), no space before X.
...
# A German 1.1.x build: the state word is the LCD language's.
...
class TestTheParsers:
    ...
    def test_m119_is_never_asked_of_a_machine_with_a_probe(self):
        ...
        Kiln asks only a build that said Cap:Z_PROBE:0."""
        asked: list[str] = []
