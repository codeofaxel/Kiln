...
# ---------------------------------------------------------------------------
# A file that already lives on the printer is read back and inspected
# ---------------------------------------------------------------------------
#
# 2026-09-16, Bambu A1: a .gcode.3mf that had sat on the SD card since
# April -- raw PrusaSlicer output wrapped without a start block -- was
# started by name.  The upload door refuses that file class (incident #0,
# 2026-04-15); the start-by-name door had nothing local to inspect and
# soft-passed.  The printer heated, never homed, never purged, and rolled
# the first extrusion into a log under the nozzle.  These pin the fix: when
# no local copy exists, the gate asks the adapter for the printer's copy
# and runs the same checks on it.
...
class _ReadsBackAdapter(_FakeAdapter):
    """A fake whose printer storage holds one file, served by read_print_file."""
...
_ReadsBackAdapter.__abstractmethods__ = frozenset()


class TestFileOnPrinterIsRead:
    def test_no_homing_on_the_printer_copy_is_refused(self):
        a = _ReadsBackAdapter(_threemf_bytes(_NO_HOMING))
        r = a.start_print("kiln_part2_disc_25x5.gcode.3mf")
        assert r.success is False
        assert a.impl_calls == []
        assert a.read_calls == ["kiln_part2_disc_25x5.gcode.3mf"]
        assert "G28" in r.message and "kiln_part2_disc_25x5.gcode.3mf" in r.message
        assert "delete" in r.message.lower()

    def test_homed_printer_copy_is_allowed(self):
        a = _ReadsBackAdapter(_threemf_bytes(_HOMED))
        r = a.start_print("good.gcode.3mf")
        assert r.success is True and a.impl_calls == ["good.gcode.3mf"]

    def test_bare_gcode_on_the_printer_is_read_the_same_way(self):
        a = _ReadsBackAdapter(_NO_HOMING.encode())
    ...
    def test_default_adapter_has_no_reader_and_soft_passes(self):
        a = _FakeAdapter()
    ...
    def test_local_copy_wins_over_the_printer_copy(self, tmp_path):
        ...
        local.write_bytes(_threemf_bytes(_NO_HOMING))
        a = _ReadsBackAdapter(_threemf_bytes(_HOMED))
        r = a.start_print("bad.gcode.3mf", local_file_path=str(local))
        assert r.success is False and a.read_calls == []

    def test_gate_reports_where_it_looked(self):
        a = _ReadsBackAdapter(_threemf_bytes(_NO_HOMING))
        blocked = pg.run_adapter_gate(a, "x.gcode.3mf", {})
        assert blocked and blocked["code"] == "NO_HOMING_SEQUENCE"
        assert blocked["inspected"] == "printer_copy"
