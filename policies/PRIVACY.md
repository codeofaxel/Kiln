...
- when you print beside a part already on the plate of a Klipper-family printer (Creality, QIDI, Voron and the like), the parts of that printer's own settings that decide how its head moves on its own — its pause, resume, cancel and homing routines and its axis limits — so the verdict is judged on how your printer really moves, and so Kiln learns how printers behave in the wild. Kiln reads your printer's own settings to learn how it moves; your prints never leave. Serial numbers, pin names, file paths and network addresses are removed before anything is sent; the board's chip name and a one-way hash of its serial are kept so one printer counts once. Same switch as the rest: `KILN_TELEMETRY=false` stops it.
