...
- which app is driving Kiln — the name and version the AI host gives at the MCP handshake (for example Claude's desktop chat or Claude Code, and for Claude Code whether it was the terminal or the desktop app's Code tab), whether that host said it can show Kiln's inline panels or ask you a question, and the AI model only when the host volunteers it (most do not, and then it is recorded as "unknown"). Never the conversation, the folder you were working in, the name you gave Kiln in that app's settings, or any session identifier
- the firmware version each registered printer reports it runs (the version text only — never a serial number or address), so Kiln can tell when printers out in the wild run a version its records were not read from
- for each Klipper-family printer you have registered (Creality, QIDI, Voron and the like), once a day with the heartbeat and again whenever you print beside a part already on its plate, the parts of that printer's own settings that decide how its head moves on its own — its pause, resume, cancel and homing routines and its axis limits — so the verdict is judged on how your printer really moves, and so Kiln learns how printers behave in the wild. Kiln reads your printer's own settings to learn how it moves; your prints never leave. Serial numbers, pin names, file paths and network addresses are removed before anything is sent; the board's chip name and a one-way hash of its serial are kept so one printer counts once. Same switch as the rest: `KILN_TELEMETRY=false` stops it.
- if you run the guided "help Kiln get to know your printer" session (`printer_bench`), what it observed your printer do — for each block it tested (pause, cancel, end), where the head went as numbers: the path from a Klipper or Marlin printer's own position readout, or the numbered zone and the lift you answered with for a closed-firmware printer, plus the two caliper figures for the head if you gave them. **Never a photo**: a camera frame is shown to you and your agent on your machine, and only your answer travels. Sent at the end of the session and after the daily heartbeat, kept once per model and distinct path (`printer_motion_observations`) with the printer model, its firmware version and the same one-way hash of its serial. With `KILN_TELEMETRY=false` nothing is sent; the numbers still ride your own print-beside requests so your printer's verdicts use them, stripped of the hash and the version and marked not to be kept.
