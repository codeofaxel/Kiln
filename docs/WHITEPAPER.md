...
5. **Agent-Native Interface.** <!-- KILN_MCP_COUNT:OLD --> 702+ typed MCP tools with structured JSON input/output make Kiln a first-class tool for any MCP-compatible agent. <!-- KILN_CLI_COUNT:OLD --> 215 CLI commands with `--json` flags cover the same surface for scripting.
