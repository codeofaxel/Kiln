...
|---|---|---|
| MCP Server (`kiln serve`) | <!-- KILN_MCP_CAPABILITY_COUNT:OLD --> 795 MCP capabilities via FastMCP | stdio (local) |
...
| E-2 | Auth scope escalation | AuthManager | API keys carry scopes (`read`, `write`, `admin`). Scope enforcement is implemented per-tool in the MCP server. A missing scope check on any single tool among <!-- KILN_MCP_TOOL_COUNT:OLD --> 788 would constitute a privilege escalation vulnerability. The large tool surface area increases the probability of inconsistent enforcement. |
...
| E-2 | Auth scope escalation | High | Medium | Per-tool scope checks; <!-- KILN_MCP_TOOL_COUNT:OLD --> 788 tools to audit | High — large surface area for missed checks |
...
2. **No fuzz testing of MCP tools.** The <!-- KILN_MCP_TOOL_COUNT:OLD --> 788 MCP tool handlers have not been subjected to automated fuzz testing for input validation edge cases, malformed JSON, or type confusion.
...
6. **No per-tool scope audit.** With <!-- KILN_MCP_TOOL_COUNT:OLD --> 788 MCP tools, there is no automated verification that every tool correctly enforces its required auth scope. Manual review is the current approach.
