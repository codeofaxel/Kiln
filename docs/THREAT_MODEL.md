...
|---|---|---|
| MCP Server (`kiln serve`) | 394 MCP tools via FastMCP | stdio (local) |
...
6. **No per-tool scope audit.** With 394 MCP tools, there is no automated verification that every tool correctly enforces its required auth scope. Manual review is the current approach.
