...
|---|---|---|
| MCP Server (`kiln serve`) | <!-- KILN_MCP_CAPABILITY_COUNT:OLD --> 903 MCP capabilities via FastMCP | stdio (local) |
...
| E-2 | Auth scope escalation | High | Medium | Per-tool scope checks; <!-- KILN_MCP_TOOL_COUNT:OLD --> 896 tools to audit | High — large surface area for missed checks |
...
2. **No fuzz testing of MCP tools.** The <!-- KILN_MCP_TOOL_COUNT:OLD --> 896 MCP tool handlers have not been subjected to automated fuzz testing for input validation edge cases, malformed JSON, or type confusion.
...
6. **No per-tool scope audit.** With <!-- KILN_MCP_TOOL_COUNT:OLD --> 896 MCP tools, there is no automated verification that every tool correctly enforces its required auth scope. Manual review is the current approach.
