...
The CLI has <!-- KILN_CLI_COUNT --> 234 commands in total; this section documents the everyday core. Run `kiln --help` for the full command tree.
...
Kiln exposes **<!-- KILN_MCP_TOOL_COUNT --> 880 MCP tools** and **<!-- KILN_MCP_CAPABILITY_COUNT --> 887 total MCP capabilities**. The everyday core is documented below by category; agents see the full, current catalog at connect time (`get_started` and `get_skill_manifest` return the complete map).
