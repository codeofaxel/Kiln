...
- **AI-native** — 340 MCP tools built for AI agents. Not a web UI with an API bolted on.
...
Tool tiers automatically match model capability: **essential** (15 tools) for smaller models, **standard** (46 tools) for mid-range, **full** (105 tools) for Claude/GPT-4/Gemini. All 340 tools are available via MCP (`kiln serve`).
...
The Kiln MCP server (`kiln serve`) exposes **340 tools** to agents. Key tools are listed below — run `kiln tools` for the complete catalog.
...
| `cli/` | Click CLI with 114 commands and JSON output |
...
# Run tests (7,777 total)
cd kiln && python3 -m pytest tests/ -v        # 7,538 tests
