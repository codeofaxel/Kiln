...
- **AI-native** — 816 MCP capabilities and 221 CLI commands built for AI agents. Not a web UI with an API bolted on.
...
Tool tiers automatically match model capability: **essential** (16 tools) for smaller models, **standard** (61 tools) for mid-range, **full** (133 tools) for stronger models. All 809 tools are available via MCP (`kiln serve`).
...
The Kiln MCP server (`kiln serve`) exposes **809 tools** to agents, plus prompts and resources for **816 total MCP capabilities**. Key tools are listed below — MCP clients can use `get_skill_manifest` and ToolSearch-style discovery to browse the complete catalog.
