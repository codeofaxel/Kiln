...
[![SafeSkill 60/100](https://img.shields.io/badge/SafeSkill-60%2F100_Use%20with%20Caution-orange)](https://safeskill.dev/scan/codeofaxel-kiln)
...
Tool tiers automatically match model capability: **essential** (15 tools) for smaller models, **standard** (46 tools) for mid-range, **full** (105 tools) for Claude/GPT-4/Gemini. All <!-- KILN_MCP_COUNT:OLD --> 567 tools are available via MCP (`kiln serve`).
...
The Kiln MCP server (`kiln serve`) exposes **<!-- KILN_MCP_COUNT:OLD --> 567 tools** to agents. Key tools are listed below — run `kiln tools` for the complete catalog.
...
| `cli/` | Click CLI with <!-- KILN_CLI_COUNT:OLD --> 165 commands and JSON output |
...
# Run tests (10,250+ total)
cd kiln && python3 -m pytest tests/ -v        # 10,000+ tests
