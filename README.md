...
Tool tiers auto-match model capability: **essential** (16 tools) for smaller models, **standard** (61 tools) for mid-range, **full** (133 tools) for stronger models. All <!-- KILN_MCP_TOOL_COUNT:OLD --> 881 tools are available over MCP via `kiln serve`.
</details>
...
- **AI-native** — <!-- KILN_MCP_CAPABILITY_COUNT:OLD --> 888 MCP capabilities and <!-- KILN_CLI_COUNT:OLD --> 235 CLI commands built for AI agents. Not a web UI with an API bolted on.
...
Kiln ships **tuned profiles for 59 models across 15 brands** — Creality, Bambu Lab, QIDI, Elegoo, Prusa Research, Sovol, AON3D, Voron, AnkerMake, Artillery, FlashForge, INTAMSYS, RatRig, SparkX, Vision Miner. [See the full searchable list →](https://kiln3d.com/printers?utm_source=github&utm_medium=readme)
...
The Kiln MCP server (`kiln serve`) exposes **<!-- KILN_MCP_TOOL_COUNT:OLD --> 881 tools** to agents, plus prompts and resources for **<!-- KILN_MCP_CAPABILITY_COUNT:OLD --> 888 total MCP capabilities**. Rather than list them all here, agents browse the live catalog with `get_skill_manifest` and ToolSearch-style discovery. A representative slice:
