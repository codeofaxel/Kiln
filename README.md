...
Tool tiers auto-match model capability: **essential** (16 tools) for smaller models, **standard** (61 tools) for mid-range, **full** (133 tools) for stronger models. All 809 tools are available over MCP via `kiln serve`.
</details>
...
- **AI-native** — 816 MCP capabilities and 222 CLI commands built for AI agents. Not a web UI with an API bolted on.
...
The Kiln MCP server (`kiln serve`) exposes **809 tools** to agents, plus prompts and resources for **816 total MCP capabilities**. Rather than list them all here, agents browse the live catalog with `get_skill_manifest` and ToolSearch-style discovery. A representative slice:
...
| `cli/` | Click CLI with <!-- KILN_CLI_COUNT:OLD --> 222 commands and JSON output |
